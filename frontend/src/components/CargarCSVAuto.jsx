...
const CargarCSVAuto = ({ onCargar }) => {
  ...
  const checkExistingQuestions = async () => {
    try {
      const response = await axios.get('http://localhost:8000/api/autoevaluacion/contar_preguntas');
      ...
      console.error('Error checking existing questions autoevaluacion', error);
  ...
  const handleUpload = async (clearExisting = false) => {
    ...
    if (clearExisting) {
      try {
        await axios.delete('http://localhost:8000/api/autoevaluacion/eliminar_todas_preguntas');
      } catch (error) {
        console.error('Error clearing existing questions autoevaluacion', error);
      }
    }
    ...
    try {
      await axios.post('http://localhost:8000/api/autoevaluacion/importar_csv', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      alert('CSV de autoevaluación importado exitosamente');
      onCargar(); // Refrescar preguntas activas
      onClose(); // Cerrar modal
    } catch (error) {
      alert('Error al importar CSV de autoevaluación');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <VStack spacing={4} align="stretch">
        ...
        <Text fontSize="sm" color="gray.600">Formato: pregunta,respuesta (una por línea)</Text>
        <Input
          type="file"
          accept=".csv"
        ...
        <ModalContent>
          ...
          <ModalBody pb={6}>
            <Alert status="info" mb={4}>
              <AlertIcon />
              Ya hay {existingQuestions?.total || 0} preguntas de autoevaluación en la base de datos
